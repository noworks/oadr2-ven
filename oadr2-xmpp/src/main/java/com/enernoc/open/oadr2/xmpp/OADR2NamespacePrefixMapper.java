...
public class OADR2NamespacePrefixMapper extends NamespacePrefixMapper {
	...
	static {
		...
		for (XMLNS ns : XMLNS.values())
			uris.add(ns.getNamespaceURI());
	...
	/**
	 * This attempts to add the namespace prefix mapper to the JAXB marshaller, so
	 * that serialized payloads will have 'standard' XML namespace prefixes. Note
	 * this is only likely to work if you're using the Sun XML APIs.
	 * 
	 * @param marshaller
	 * @throws PropertyException
	 */
	public void addTo(Marshaller marshaller) throws PropertyException {
		// System.out.println("--------------- Marshaller class: " +
		// marshaller.getClass().toString());
	...
	public String getPreferredPrefix(String namespaceUri, String suggestion, boolean requirePrefix) {
		// System.out.println("----------------- Asked for namespace for " +
		// namespaceUri);
		try {
			XMLNS ns = XMLNS.fromURI(namespaceUri);
			if (ns == XMLNS.OADR2 && !requirePrefix)
				return "";
			return ns.getPrefix();
