...
/**
 * Wraps {@link Unmarshaller} and provide a functionality to unmarshal from a
 * {@link XmlPullParser}.
 * 
 * <p>
 * This is particularly useful to unmarshal just a part of a document into a
 * JAXB object.
 * 
 * <p>
 * It is often the case that you don't need complete random access to the entire
 * document. Instead, you usually just need random access to a limited region of
 * XML.
 * 
 * In this scenario, it is typically a waste of resources to bind the entire
 * document into a single JAXB object tree. It will consume more memory, it will
 * cause a greater latency.
 * 
 * <p>
 * In some cases, your interesting XML document is contained inside an
 * "envelope", which you are not so interested in.
 * 
 * In this scenario, this class can be used to just bind the real meat to a JAXB
 * object. In this way, you don't need to compile a schema that corresponds to
 * the envelope.
 * 
 * <h2>Example</h2>
 * <p>
 ...
 * 
 * <p>
 * Suppose the address book is very long and your program is just looking for
 * the record of "Jane Smith". Unmarshalling the whole document is easy, but it
 * forces the entire document to be loaded, which consumes more memory.
 * 
 * <p>
 ...
 * 
 * <p>
 * Note that the unmarshaller will think that the &lt;contact> element is the
 * root element of a document, so it is important for the &lt;contact> element
 * to be declared as a global element.
 * 
 * <p>
 * It unmarshals one &lt;contact> at a time, then process it. The added benefit
 * is that you can stop the processing as soon as you find the info you are
 * looking for. This makes the program a lot (average of roughly 2x) faster.
 * 
...
public final class PullUnmarshaller {
	...
	/**
	 * Constrcuts a {@link PullUnmarshaller} by wrapping an Unmarshaller instance.
	 * <p>
	 * All the real work of unmarshalling is done through the specified
	 * unmarshaller. So configure it appropriately and don't reuse this unmarshaller
	 * instance while this {@link PullUnmarshaller} is in use.
	 */
	 ...
	 * <p>
	 * the parser must point to a start element. This method will read the whole
	 * sub-tree, send it to the JAXB unmarshaller (which was specfied to the
	 * constructor), then return the obtained object.
	 * <p>
	 * After the successful completion of this method, the parser will point to the
	 * event next to the end element of the sub-tree.
	 * <p>
	 * If a processing fails, the parser will be left at somewhere in the middle of
	 * the sub-tree.
	 * 
	 * 
	 * @param parser
	 *            A pull parser from which an object will be unmarshalled. The
	 *            parser must be at the start element. IOW,
	 *            <code>pp.getEventType() == XmlPullParser.START_TAG</code> has to
	 *            be true.
	 * 
	 * @return Always return a non-null valid object.
	 * 
	 * @throws IllegalStateException
	 *             If the parser is not set at a start element event, this exception
	 *             will be thrown.
	 * 
	 * @throws JAXBException
	 *             In case of any XML wellformedness error, IO error, and JAXB
	 *             unmarshalling errors. Before this exception is thrown, it is
	 *             reported to the {@link javax.xml.bind.ValidationEventHandler} of
	 *             the unmarshaller first.
	 */
	...
	class Worker {
		 ...
		 * 
		 * A parser has to be at the START_TAG event, and when this method leaves, the
		 * parser is at the corresponding END_TAG event.
		 */
		...
		/**
		 * {@link Attributes} implementation that uses the {@link XmlPullParser} as the
		 * real data storage.
		 */
		private final Attributes attributes = new Attributes() {
			...
			public String getQName(int idx) {
				...
				if (prefix == null)
					...
					return prefix + ':' + pp.getAttributeName(idx);
			}
		...
		/**
		 * {@link Locator} implementation that uses the {@link XmlPullParser} as the
		 * real data storage.
		 */
