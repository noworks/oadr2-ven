...
/**
 * Packet extensions are created by XMPP extension implementations that
 * understand a particular namespace that appears in the body of an XMPP packet
 * (message or IQ.) In this case, this extension will be created whenever the
 * OpenADR 2.0 namespace (http://openadr.org/oadr-2.0[a or b]/2012/07) is
 * encountered. It's also used to serialize (send) a packet that contains an
 * OpenADR payload.
 * 
 * @see PacketExtension
 * @author tnichols
...
public class OADR2PacketExtension implements PacketExtension {
	...
	/**
	 * This will return the parsed OpenADR model object which will be one of:
